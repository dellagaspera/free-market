...
int buscaUsuario(usuario_t *usuario, int nUsuarios) {
    ...
    for(int i = 0; i < nUsuarios; i++) {
        printf("%d - %s\n", (i+1), usuario[i].nome);//Muda esse %d dps pra %s, ai vai ser o id seguido do nome, assim-> id - nome
    }

    char tempVar[MAX_CHAR_ID];
    printf("Digite o nome do usuario(vai ser id dps tropa):");
...
void cadastraProduto(usuario_t * usuario, produto_t ** PRODUTOS, int nProdutos) { 
    ...
    // Coloca o ID como a quantidade de produtos
    usuario->produtos[usuario->nProdutos].Id = usuario->nProdutos;
...
erro_t favoritarProduto(usuario_t *usuarios, int nUsuarios, int idUsuarioAtual) {
    ...
    usuarioAtual->produtosFavoritos = (int *)realloc(usuarioAtual->produtosFavoritos, (usuarioAtual->nProdutos + 1) * sizeof(int));
    usuarioAtual->produtosFavoritos[usuarioAtual->nProdutos] = //;
    usuarioAtual->nProdutos++;
...
int telaInicial(usuario_t * Usuarios, int loginAtual) {
    ...
    if (loginAtual != USUARIO_SEM_LOGIN) {
        ...
        } while (escolha < 0 || escolha > 10); // Enquanto o usuário não tiver digitado algo válido
